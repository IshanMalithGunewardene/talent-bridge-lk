//#region node_modules/@supabase/auth-ui-shared/dist/index.mjs
var ThemeSupa = {
	default: {
		colors: {
...
function generateClassNames(classNameKey, defaultStyles, appearance) {
	...
	if ((_a = appearance == null ? void 0 : appearance.className) == null ? void 0 : _a[classNameKey]) classNames.push((_b = appearance == null ? void 0 : appearance.className) == null ? void 0 : _b[classNameKey]);
	if ((appearance == null ? void 0 : appearance.extend) === void 0 || (appearance == null ? void 0 : appearance.extend) === true) classNames.push(defaultStyles);
	return classNames;
}
...
function value(src, next) {
	...
	if (src && next && typeof src === "object" && typeof next === "object") {
		if (Array.isArray(next)) for (k = 0; k < next.length; k++) src[k] = value(src[k], next[k]);
		else for (k in next) src[k] = value(src[k], next[k]);
		return src;
	}
	return next;
}
function merge(target, ...args) {
	let len = args.length;
	for (let i = 0; i < len; i++) target = value(target, args[i]);
	return target;
}
...
//#endregion
export { CLASS_NAMES, PREPENDED_CLASS_NAMES, SocialLayouts, ThemeMinimal, ThemeSupa, VIEWS, darkThemes, en_default as en, generateClassNames, merge, minimal, supabase, template };
